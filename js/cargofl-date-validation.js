...
$(document).on('changeDate', '.cargofl-date-validation', function (event) {
	...
	// check if this date is less than current date date
    ...
    // Checks if provided date is greater than current date
	if ($(this).hasClass('cargofl-disable-future-date')) {
		if (event.date && moment(event.date).isAfter(moment(), 'day')) {
            validation_error = (field_label ? field_label : 'Date') + " should not be greater than current date.";
			alert(validation_error);
			$(this).val('').focus();
			return;
		}
